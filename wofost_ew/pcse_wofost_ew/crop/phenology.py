...
In this implementation, Zheng et al. (2024) enhanced the phenological development model
by adding an extreme weather function F(EW), which improves the model's ability to simulate
crop growth under extreme weather conditions.
Jinhui Zheng (zhengjinhui@igsnrr.ac.cn), July 2024
"""
import datetime

from ..traitlets import Float, Int, Instance, Enum, Bool, Unicode
...
class DVS_Phenology(SimulationObject):
    ...
    **Signals sent or handled**
    ...
    _extreme_weather_function_calculated = False
    EXTREME_WEATHER_FUNCTION = None
    ...
    class Parameters(ParamTemplate):
        ...
        HDD = Float(-99.)  # High-temperature degree days
        LDD = Float(-99.)  # Low-temperature degree days
        R95P = Float(-99.)  # Very wet days
        R10mm = Float(-99.)  # Heavy precipitation days
        Rx1day = Float(-99.)  # Max 1-day precipitation amount
        PDSI = Float(-99.)  # Palmer Drought Severity Index
        VPD = Float(-99.)  # Vapor pressure deficit
        LSTM_MODEL_PATH = Unicode()  # Trained LSTM model for EXTREME_WEATHER_FUNCTION prediction
    ...
    def calc_rates(self, day, drv):
        ...
        if not self._extreme_weather_function_calculated:
            # Prepare input features for LSTM model
            extreme_weather_features = np.array((
                p.HDD,
                p.LDD,
                p.R95P,
                p.R10mm,
            ...
            extreme_weather_features = np.array(extreme_weather_features).reshape((1, 1, 7))

            # Predict EXTREME_WEATHER_FUNCTION using the LSTM model
            LSTM_MODEL = load_model(p.LSTM_MODEL_PATH, compile=False)
            self.EXTREME_WEATHER_FUNCTION = LSTM_MODEL.predict(extreme_weather_features)

            # Ensure EXTREME_WEATHER_FUNCTION is within valid range [0, 1]
            self.EXTREME_WEATHER_FUNCTION = max(0.0, min(1.0, self.EXTREME_WEATHER_FUNCTION))
            self._extreme_weather_function_calculated = True

        EXTREME_WEATHER_FUNCTION = self.EXTREME_WEATHER_FUNCTION
        ...
        if s.STAGE == "emerging":
            ...
            r.DTSUME = 0.
            r.DTSUM = p.DTSMTB(
                drv.TEMP) * VERNFAC * DVRED * EXTREME_WEATHER_FUNCTION  # The extreme weather function operates during the vegetative period.
            r.DVR = r.DTSUM / p.TSUM1

        elif s.STAGE == 'reproductive':
            r.DTSUME = 0.
            r.DTSUM = p.DTSMTB(
                drv.TEMP) * EXTREME_WEATHER_FUNCTION  # The extreme weather function operates during the reproductive period.
